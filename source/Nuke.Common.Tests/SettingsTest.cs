...
namespace Nuke.Common.Tests
{
    public class SettingsTest
    {
        private static AbsolutePath RootDirectory =>
            Constants.TryGetRootDirectoryFrom(Directory.GetCurrentDirectory()).NotNull();
        ...
        public void TestCommon()
        {
            ...
            var logEntry = default((OutputType Type, string String, List<ConsoleColor>));
            ...
            settings.ProcessEnvironmentVariables
                .Should()
                .ContainSingle(x => x.Key == "key" && x.Value == "value");
            settings.ProcessExecutionTimeout.Should().Be(1_000);
            settings.ProcessArgumentConfigurator
                .Invoke(new Arguments())
                .RenderForOutput()
                .Should()
                .Be("/switch");
            logEntry
        ...
        public void TestMSBuild()
        {
            ...
            Assert<MSBuildSettings>(
                x =>
                    x.SetProjectFile(projectFile)
        ...
        {
            Assert<Xunit2Settings>(
                x =>
                    x.AddTargetAssemblies("A.csproj")
                        .AddTargetAssemblyWithConfigs(
                            "B.csproj",
                            "D.config",
                            "new folder\\E.config"
                        ),
            ...
            Assert<Xunit2Settings>(
                x =>
                    x.AddResultReport(Xunit2ResultFormat.HTML, "new folder\\data.html")
            ...
            Assert<Xunit2Settings>(
                x =>
                    x.AddResultReport(Xunit2ResultFormat.NUnit, "new folder\\nunit.xml")
                        .EnableDiagnostics()
                        .EnableFailSkips(),
        ...
        public void TestOpenCover()
        {
            ...
            Assert<OpenCoverSettings>(
                x =>
                    x.SetTargetPath(projectFile)
